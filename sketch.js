...
function preload() {
  ...
  spritesheet_newChar = loadImage('腳色2STOP/ALL699190.png');
  spritesheet_newChar_laugh = loadImage('腳色2笑/ALL5851830006.png'); // 請確保這個路徑是正確的
  spritesheet_newChar_hit = loadImage('腳色2倒/ALL2959156.png');
...
function setup() {
  ...
  // 初始化角色位置在畫布中央
  characterX = width / 2;
  groundY = height * 0.8; // 將地面Y座標往下移，讓角色和背景更協調
  characterY = groundY;

  // 初始化新角色位置在原本角色的左邊
  newCharX = width / 2 - 200; // newCharX 也應該是相對於世界的位置
  newCharY = groundY;
...
function draw() {
  ...
  // 計算角色2在螢幕上的實際位置
  let worldNewCharX = newCharX + bgX;

  // --- 更新和繪製投射物 ---
  // 只有在角色2沒被擊倒時才更新和繪製投射物
  handleProjectiles(worldNewCharX);
  ...
  if (!isNewCharHit) { 
    ...
    // --- 判斷是否觸發笑的動畫 ---
    let distance = dist(characterX, characterY, worldNewCharX, newCharY);
    // 當靠近且對話未開始時，觸發提問
  ...
  // --- 繪製新增的角色 ---
  push();
  translate(worldNewCharX, newCharY);
  ...
  // 2. 如果沒被擊中，才判斷是否在笑
    ...
    if (dialogueState === 1) {
      ...
      // 顯示輸入框在角色1頭上（確保輸入框層級高於 canvas、可輸入）
      nameInput.show();
      // 設定大小與絕對定位，避免在某些瀏覽器被隱藏或位置錯亂
      nameInput.size(200, 24);
      nameInput.style('position', 'absolute');
      nameInput.position(characterX - 100, characterY - spriteHeight_walk / 2 - 40);
      nameInput.style('z-index', '9999');
    ...
    if (newCharLaughFrameCount >= numFrames_newChar_laugh) {
      ...
      // 不自動結束 dialogueState，直到角色1(主角)離開一定距離才隱藏對話框
      if (dialogueState === 1) {
        if (typeof distance !== 'undefined' && distance >= proximityThreshold) {
          dialogueState = 0;
          nameInput.hide();
        }
        // 否則維持 dialogueState === 1，nameInput 保持顯示
      }
    }
  }
  // 3. 如果也沒在笑，就播放待機動畫
  ...
  // 2. 如果不在攻擊中，才處理移動和跳躍
    ...
    bgX += characterSpeed; // 背景向右移動
    characterDirection = -1; // 更新方向為左

    // 讓背景循環滾動
    if (bgX >= width) { bgX = 0; }

    // 根據是否在跳躍選擇動畫並翻轉 (角色本身不移動)
    push();
    translate(characterX, characterY);
    scale(-1, 1);
    if (isJumping) {
...
// 更新和繪製所有投射物
function handleProjectiles(worldNewCharX) {
  ...
  for (let i = projectiles.length - 1; i >= 0; i--) {
    ...
    let worldProjectileX = p.x + bgX; // 計算投射物在螢幕上的實際位置

    // --- 碰撞偵測 (只有在角色2還沒被擊中時才進行) ---
    if (!isNewCharHit) {
      // 建立一個簡易的碰撞偵測，檢查投射物是否進入角色2的身體範圍
      let hit = (worldProjectileX > worldNewCharX - frameWidth_newChar / 2) && (worldProjectileX < worldNewCharX + frameWidth_newChar / 2);
      if (hit) {
    ...
    // 如果投射物飛出畫面，也將其移除
    if (worldProjectileX > width || worldProjectileX < 0) {
    ...
    push();
    translate(worldProjectileX, p.y);
    scale(p.direction, 1);
